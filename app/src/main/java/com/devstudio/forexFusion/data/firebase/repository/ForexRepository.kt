...
class ForexRepository {
     ...
     **/
    private val reference = database.child(FirebaseNodes.Forex.node)
     ...
     */
    suspend fun get(onSuccess: (List<ForexSignal>) -> Unit, onFailure: (Exception) -> Unit) {
        reference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val signals = mutableListOf<ForexSignal>()
                snapshot.children.forEach {
                    val signal = it.getValue(ForexSignal::class.java)
                    if (signal != null) {
                        signals.add(signal)
                    }
                }
