...
class EventRepository {
     ...
     **/
    private val reference = database.child(FirebaseNodes.Event.node)
     ...
     */
    suspend fun get(onSuccess: (List<EventSignal>) -> Unit, onFailure: (Exception) -> Unit) {
        reference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val signals = mutableListOf<EventSignal>()
                snapshot.children.forEach {
                    val signal = it.getValue(EventSignal::class.java)
                    if (signal != null) {
                        signals.add(signal)
                    }
                }
