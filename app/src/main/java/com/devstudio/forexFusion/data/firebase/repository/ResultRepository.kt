...
class ResultRepository {
     ...
     **/
    private val reference = database.child(FirebaseNodes.Results.node)
     ...
     */
    suspend fun get(onSuccess: (List<ResultSignal>) -> Unit, onFailure: (Exception) -> Unit) {
        reference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val signals = mutableListOf<ResultSignal>()
                snapshot.children.forEach {
                    val signal = it.getValue(ResultSignal::class.java)
                    if (signal != null) {
                        signals.add(signal)
                    }
                }
